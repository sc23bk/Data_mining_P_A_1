...
class Section1:
    ...
    def partC(
        ...
        results=u.train_simple_classifier_with_cv(Xtrain=X,ytrain=y,clf=DecisionTreeClassifier(random_state=self.seed),cv=KFold(n_splits=5,shuffle=True,random_state=self.seed))
        answer = {}
    ...
    def partD(
        ...
        results=u.train_simple_classifier_with_cv(Xtrain=X,ytrain=y,clf=DecisionTreeClassifier(random_state=self.seed),cv=ShuffleSplit(n_splits=5,random_state=self.seed))
        answer = {}
    ...
    def partE(
        ...
        for k in [2,5,8,16]:
            ...
            results=u.train_simple_classifier_with_cv(Xtrain=X,ytrain=y,clf=DecisionTreeClassifier(random_state=self.seed),cv=ShuffleSplit(n_splits=k,random_state=self.seed))
            cv_dict={}
            cv_dict['mean_accuracy']=results['test_score'].mean()
            cv_dict['std_accuracy']=results['test_score'].std()
            answer[k]={}
    ...
    def partF(
        ...
        results=u.train_simple_classifier_with_cv(Xtrain=X,ytrain=y,clf=RandomForestClassifier(random_state=self.seed),cv=ShuffleSplit(n_splits=5,random_state=self.seed))
